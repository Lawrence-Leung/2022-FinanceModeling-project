...
motion = zeros(1, []);              %股价差有效数据
motion2 = zeros(1, []);             %成交量差有效数据
    %%
        % newtradematrix 矩阵说明：
        % 第1维度：每一天的情况
        % 第2维度：
...
XThirdResult(termindex) = Termindex (termindex, 7);
...
%k3 = k3 + 1
k4 = 0;     %终极大样本处理
%TrainSourceStatic4 = zeros([], 10);
%TrainTeachStatic4 = zeros([], 1);
InputMatrixOrigin = zeros([], 10);
DealMatrixOrigin = zeros([], 10);
Input11ArrayOrigin = zeros([], 1);
Input11DealOrigin = zeros([], 1);
for i = 1 : size(motion, 2) - 11 - 1
    k4 = k4 + 1;
    for j = 1 : 10
    InputMatrixOrigin(k4, j) = motion(i + j - 1);
    DealMatrixOrigin(k4, j) = motion2(i + j - 1);
    end
    Input11ArrayOrigin(k4, 1) = motion(i + 10);
    Input11DealOrigin(k4, 1) = motion2(i + 10);
end
k4
...
%%如果要跑一天数据，记得改PackUpJudgeHistory的3天哦

subplot(11, 3, iplot001)
bar(AveProfitRate)
grid on;
xlabel('交易次数');
ylabel('收益率');

tempProfitRate = mean(AveProfitRate)        %平均收益率
string1 = append(VarName2(iplot001), ' ','收益率',string(tempProfitRate));
    string1
    title(string1);
origin(3,1) %前一天的收盘价
